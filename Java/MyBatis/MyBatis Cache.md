...
- 清除一级缓存需要使用`clearCache()`
  - `clearCache[SqlSession]` -> `clearCache[DefaultSqlSession]` -> `clearLocalCache[Executor]` -> `clearLocalCache[BaseExecutor]` -> `clear[PerpetualCache]`
...
- 为什么使用`MyBatis`时， `Pojo`类需要实现`Serializable`接口
  - 开启了二级缓存后，还需要将缓存的`Pojo`实现`Serializable`接口，为了将缓存数据取出执行反序列化操作，因为二级缓存数据存储介质多种多样，不一定只存在内存中，有可能存在硬盘中，如果我们再取这个缓存的话，就需要反序列化了
  - 使用`useCache`决定是否禁用二级缓存 `@options(usechae=true)`
  - `flushCache`用于`insert`，`update`，`delete`操作之后刷新缓存
