...
- 页面切换路由
- 页面所在的 tab 标签重新变得可见
针对上述的 3 种场景，特别是切换路由的情况，如果切换路由是通过改变$hash$值来实现的，那么只需要监听$ hashchange$事件，如果是通过$html5$的$history api$ 来改变$url$的，那么需要重新定义$pushstate$和 $replacestate$事件。
...
## 如何上报性能数据
- 通过 ajax 请求的形式来上报前端性能数据有一些缺陷
- 必须对跨域做特殊处理
- 如果页面销毁后，相应的 ajax 方法并不一定发送成功
- 解决方式：
...
2. 动态创建 img 标签的形式
    ...
    通过动态创建 img 标签的形式，指定 src 属性所指定 的 url 来发送请求，首先不受跨域的限制，其次 img 标签动态插入，会延迟页面的卸载保证图片的插入，因此可以保证在页面的销毁期，请求可以发生。
    ...
    ```
    在 get 请求的处理中，我们通过 res.sendFile(join(\_\_dirname, '..', 'public/1.jpg'))将图片返回后，这样前端的 image 的 onload 方法才会被调用。
